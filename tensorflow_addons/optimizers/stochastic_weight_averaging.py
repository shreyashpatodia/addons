...
averaging hias shown to improve model performance on validation/test sets whilst
possibly causing a small increase in loss on the training set.
...
class SWAOptimizer(tf.keras.optimizers.Optimizer):
    ...
    deviation of the activations for each layer of the network with w_SWA weights
    ...
    pass over the data in training mode ([Averaging Weights Leads to Wider Optima
    ...
    def __init__(self,
        ...
        Args:
                ...
                start_averaging: An integer. Threshold to start averaging using SWA.
                    Averaging only occurs at `start_averaging` iterations, must be >= 0.
                    If start_averaging = m, the first snapshot will be taken after the mth
                    application of gradients (where the first iteration is iteration 0).
                average_period: An integer. The synchronization period of SWA. The
                    averaging occurs every average_period steps. Averaging period needs to
                    be >= 1.
                name: Optional name for the operations created when applying gradients.
                    Defaults to 'SWAOptimizer'.
    ...
    def _average_op(self, var):
        ...
        # number of times snapshots of weights have been taken (using max to avoid
        # negative values of num_snapshots).
        num_snapshots = tf.math.maximum(
                0, tf.math.floordiv(self.iterations - start_averaging, average_period))
        ...
        average_value = (average_var * num_snapshots + var) / (num_snapshots + 1.)
    ...
    def assign_average_vars(self, var_list):
        ...
        Returns:
            assign_op: The op corresponding to the assignment operation of variables
                to their average.
